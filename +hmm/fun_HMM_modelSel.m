...
switch METHOD
    ...
    case 'BIC'
        % note that on December 3rd, 2022, Luca changed the penalty term
        % here
        % previously the number of parameters estimated by the model was:
        % k = NumStates.*(NumStates-1) + NumStates.*gnunits
        % he has since changed it to:
        % k = NumStates.*(NumStates-1) + NumStates.*gnunits + NumStates - 1
