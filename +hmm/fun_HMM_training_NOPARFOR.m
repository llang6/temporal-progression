...
for ix=1:NumIter
    ...
    for it_cnt=1:NumRuns % run NumRuns times unless we reach tolerance tol<TOL
        ...
        if it_cnt==1 % first run uses initial guesses
            ...
            %[tmp(ix).esttr,tmp(ix).estemis,tmp(ix).logliks]=hmm.hmmtrain(seq,tmp(ix).trguess,tmp(ix).emisguess,'Maxiterations',1,'Algorithm','Viterbi');
        elseif it_cnt>1
            ...
            %try
            %    [tmp(ix).esttr,tmp(ix).estemis,tmp(ix).logliks]=hmm.hmmtrain(seq,tmp(ix).esttr,tmp(ix).estemis,'Maxiterations',1,'Algorithm','Viterbi');
            %catch ME
            %    tr = tmp(ix).esttr 
            %    e = tmp(ix).estemis
            %    fprintf('ix=%i\n',ix)
